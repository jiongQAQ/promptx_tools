...
**触发条件**: 若 `<projectRoot>/paper/reference-structure.json` 存在

**加载步骤**:
1. 读取 `reference-structure.json` 文件
2. 提取以下风格特征：
   - `terminology.technical_terms`: 专业术语字典
   - `terminology.high_frequency_terms`: 高频术语列表
   - `writing_style.figure_reference_format`: 图表引用格式
   - `writing_style.table_reference_format`: 表格引用格式
   - `writing_style.technical_detail_level`: 技术细节深度
   - `writing_style.academic_tone`: 学术语气

**应用方式**:
- **术语一致性**: 优先使用参考论文的专业术语和缩写
- **表达方式**: 模仿参考论文的段落组织和学术表达
- **图表引用**: 使用参考论文的引用格式（如"如图X-Y所示"）
- **技术深度**: 匹配参考论文的技术描述详细程度
...
**Step 0: 加载参考风格（可选）**
```bash
# 若存在 reference-structure.json，先读取
Read({
  "file_path": "<projectRoot>/paper/reference-structure.json"
})
# AI会自动提取术语和风格特征用于后续生成
```
...
**注**: 若 Step 0 加载了参考风格，AI在 Step 3 生成内容时会自动应用这些风格特征
