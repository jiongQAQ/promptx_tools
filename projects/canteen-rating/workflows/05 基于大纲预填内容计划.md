...
5) 参考论文结构：./paper/reference-structure.json（可选，若存在则作为最高优先级规则来源）
...
### 识别原则（规则优先级从高到低）
1. **参考论文规则**：若 `./paper/reference-structure.json` 存在，优先使用其规则
   - 加载 `image_distribution` 和 `special_patterns`
   - 匹配相同或相似章节标题
   - 应用参考论文的图表类型和命名规范
2. **章节名称语义分析**：基于章节 title 的关键词判断图表需求
3. **层级深度判断**：根据章节深度（id 层级）推断内容类型
4. **源码证据映射**：结合源码结构自动推断合适的图表类型
...
### 层级深度规则（可被参考论文规则覆盖）
- **一级章节**（id 如"3"、"4"）：默认仅 text，除非标题含明确图表关键词或参考论文规则覆盖
- **二级章节**（id 如"3.1"、"4.2"）：
  - 有三级子节点 → 默认不写 text（但参考论文规则可覆盖）
...
### 参考论文规则加载机制
若 `./paper/reference-structure.json` 存在，执行以下步骤：

#### 1. 加载参考结构
```bash
Read({ "file_path": "./paper/reference-structure.json" })
```

#### 2. 提取关键规则
- `special_patterns.parent_with_content`: 父章节内容规则
- `special_patterns.implementation_structure`: 实现章节命名模式
- `special_patterns.usecase_structure`: 用例章节命名模式
- `special_patterns.dfd_hierarchy`: 数据流图分层模式
- `image_distribution`: 图表分布和类型映射
- `table_distribution`: 表格分布映射
- `word_count_analysis`: 字数分配参考

#### 3. 应用规则（优先级最高）
- **父章节内容规则**: 若当前章节 id 或 title 匹配 `parent_with_content` 中的模式，即使有子节点也添加 text+图
- **图表类型选择**: 优先匹配 `image_distribution` 中相同章节 id 或相似 title 的图表类型
- **命名模式**: 遵循参考论文的图表命名规范（如 `dfd-{module}-level2.png`）
- **字数分配**: 参考 `word_count_analysis` 的层级和位置字数范围

#### 4. 标注来源
在生成的 content-plan 中添加注释字段 `_reference_source: "参考 XXX.md 结构"`
...
### 覆盖规则（优先级从高到低）
1. **参考论文驱动（最高优先级）**：
   - 若 `./paper/reference-structure.json` 存在且匹配到相应章节规则
   - 应用 `special_patterns.parent_with_content` 中的模式
   - 若参考论文中对应章节有内容，则当前章节也添加内容

2. **关键词触发**：若章节标题含以下关键词，即使有子节点也应写 text+图：
   - "访问流程"、"系统流程"、"数据流"
   - "逻辑设计"、"架构"、"结构"
